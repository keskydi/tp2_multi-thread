...
//Collection de points ! 
	...
	public DrawPanel() {
		...
		for (int i = 0; i < 8; i++) {
			Point point = new Point(ThreadLocalRandom.current().nextInt(0, 680),
					ThreadLocalRandom.current().nextInt(0, 480), pointerSize, pointerColor, pointerType);
		...
		this.addMouseListener(new MouseAdapter() {
			public void mousePressed(MouseEvent e) {
				Meat object = new Meat(new Point(e.getX() - (pointerSize / 2), e.getY() - (pointerSize / 2),
						pointerSize, Color.blue, pointerType));
				// object.start();
				points.add(object);
